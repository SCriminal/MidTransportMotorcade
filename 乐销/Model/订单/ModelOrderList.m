...
NSString *const kModelOrderListIsDispatch = @"isDispatch";
NSString *const kModelOrderListTotalAccept = @"totalAccept";
...
+ (NSString *)transformOrderStatusToCompleteName:(int)state {
    switch ((int)state) {
        case 401:
            return @"派单";
            break;
...
+ (ENUM_ORDER_OPERATE_TYPE)transformOrderOperateType:(int)status{
    switch (status) {
        case 401:
            ...
            break;
        case 402:
            return ENUM_ORDER_OPERATE_WAIT_TRANSPORT;
            break;
        case 410:
...
- (instancetype)initWithDictionary:(NSDictionary *)dict {
    ...
    if (self && [dict isKindOfClass:[NSDictionary class]]) {
        ...
        self.isDispatch = [dict doubleValueForKey:kModelOrderListIsDispatch];
        self.totalAccept = [dict doubleValueForKey:kModelOrderListTotalAccept];
...
- (NSDictionary *)dictionaryRepresentation {
    ...
    [mutableDict setValue:[NSNumber numberWithDouble:self.isDispatch] forKey:kModelOrderListIsDispatch];
    [mutableDict setValue:[NSNumber numberWithDouble:self.totalAccept] forKey:kModelOrderListTotalAccept];
