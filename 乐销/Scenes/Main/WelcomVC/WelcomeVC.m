...
-(UIButton *)btnCenter{
    if (_btnCenter == nil) {
        ...
        _btnCenter.rightBottom = XY(SCREEN_WIDTH - W(13), self.bottomView.top - W(17));
    }
    return _btnCenter;
}
- (WelcomeBottomView *)bottomView{
    if (!_bottomView) {
        ...
        _bottomView.blockClick = ^{
            [GB_Nav popLastAndPushVC:[LoginViewController new]];
        };
    }
    return _bottomView;
}
...
- (void)resetViewWithModel:(id)model{
    ...
    self.ivSurround.rightCenterY = XY(self.ivGift.left ,self.ivGift.centerY);
    
    self.ivOrder.leftCenterY = XY(self.ivGift.right ,self.ivGift.centerY);
    
    //设置总高度
    self.height = self.ivGift.bottom + W(20) + iphoneXBottomInterval;
}
