...
@interface DriverFilterView ()<UITextFieldDelegate>

@end

@implementation DriverFilterView
...
- (void)resetViewWithModel:(id)model{
    ...
    self.viewBG.centerXTop = XY(SCREEN_WIDTH/2.0,NAVIGATIONBAR_HEIGHT);
    {
        ...
        [viewBorder addTarget:self action:@selector(tfBillClick)];
...
- (void)btnSearchClick{
    if (self.blockSearchClick) {
        self.blockSearchClick(self.tfBillNo.text,self.tfPhone.text);
    }
    [GlobalMethod endEditing];
    [self removeFromSuperview];
}
